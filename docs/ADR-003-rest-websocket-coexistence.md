...
En el Lab 6 necesitamos colaboración en tiempo real. Ya teníamos REST del Lab 5
...
- **REST** — para todo lo que necesita una respuesta directa: crear un board, cargarlo y guardar el snapshot. Si algo falla, el cliente lo sabe de inmediato.
...
El servidor nunca retransmite a ciegas — todo evento pasa por `BoardEventApplicationService` antes del broadcast. Si la validación falla, no se propaga nada.
...
- Si el WebSocket falla, igual se puede crear y cargar un board.
